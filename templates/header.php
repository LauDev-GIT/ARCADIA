
<?php

use App\Entity\User;
use App\Tools\NavigationTools;
//adminOnly();
?>
<!DOCTYPE html> 
<html lang="fr-FR">
  <head>
    <meta charset="UTF-8" /> 
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title> Arcadia</title>
    <link rel="stylesheet" href="./assets/css/projet_reset.css" />
    <link rel="stylesheet" href="./assets/css/projet.css" />
  </head>
  <body>

    
    <header class="header flux">
      <img
        class="header__logo"
        src="./assets/images/image_svg/ARCADIA.svg"
        alt="logo_de_l'entreprise"
      />
      <button class="header__burger">
       
        <svg
          width="35"
          height="36"
          viewBox="0 0 35 36"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M5.46875 26.75C5.46875 26.4599 5.58398 26.1817 5.7891 25.9766C5.99422 25.7715 6.27242 25.6562 6.5625 25.6562H28.4375C28.7276 25.6562 29.0058 25.7715 29.2109 25.9766C29.416 26.1817 29.5312 26.4599 29.5312 26.75C29.5312 27.0401 29.416 27.3183 29.2109 27.5234C29.0058 27.7285 28.7276 27.8438 28.4375 27.8438H6.5625C6.27242 27.8438 5.99422 27.7285 5.7891 27.5234C5.58398 27.3183 5.46875 27.0401 5.46875 26.75ZM5.46875 18C5.46875 17.7099 5.58398 17.4317 5.7891 17.2266C5.99422 17.0215 6.27242 16.9062 6.5625 16.9062H28.4375C28.7276 16.9062 29.0058 17.0215 29.2109 17.2266C29.416 17.4317 29.5312 17.7099 29.5312 18C29.5312 18.2901 29.416 18.5683 29.2109 18.7734C29.0058 18.9785 28.7276 19.0938 28.4375 19.0938H6.5625C6.27242 19.0938 5.99422 18.9785 5.7891 18.7734C5.58398 18.5683 5.46875 18.2901 5.46875 18ZM5.46875 9.25C5.46875 8.95992 5.58398 8.68172 5.7891 8.4766C5.99422 8.27148 6.27242 8.15625 6.5625 8.15625H28.4375C28.7276 8.15625 29.0058 8.27148 29.2109 8.4766C29.416 8.68172 29.5312 8.95992 29.5312 9.25C29.5312 9.54008 29.416 9.81828 29.2109 10.0234C29.0058 10.2285 28.7276 10.3438 28.4375 10.3438H6.5625C6.27242 10.3438 5.99422 10.2285 5.7891 10.0234C5.58398 9.81828 5.46875 9.54008 5.46875 9.25Z"
            fill="white"
          />
        </svg>
      </button>
      <nav class="header__nav">
        <ul class="header__list-links">
          <li><a href="/index.php <?= NavigationTools::addActiveClass('page', 'home') ?>">Accueil</a></li>
          
          <?php if (User::isAdmin()) { ?>
          
          
          <li><a href="App/admin/templates/index.php<?= NavigationTools::addActiveClass('user', 'registerAdmin') ?>">Administrateur</a></li>
          
          <?php } else { ?>
            <li> <a href="index.php?controller=page&action=about <?= NavigationTools::addActiveClass('page', 'about') ?>">Contact</a></li>
       
           <?php } ?>
          
          <?php if (User::isLogged()) { ?>
            <li><a href="index.php?controller=auth&action=logout">Déconnexion</a></li>
            <li><a href="?controller=user&action=employee<?= NavigationTools::addActiveClass('auth', 'login') ?>">Employee</a></li>
            <li><a href="?controller=user&action=veterinaire<?= NavigationTools::addActiveClass('auth', 'login') ?>">Veterinaire</a></li>
                 
            <?php } else { ?>
                  <li><a href="index.php?controller=auth&action=login<?= NavigationTools::addActiveClass('auth', 'login') ?>">Connexion</a></li>
                    
                    <li><a href="index.php?controller=user&action=register<?= NavigationTools::addActiveClass('user', 'register') ?>">Inscription</a></li>
                    
                <?php } ?>  
        </ul>
      </nav>
    </header>
    <main class="main flux">